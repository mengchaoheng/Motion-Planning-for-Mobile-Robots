    ...
    for(int i = -1;i <= 1;++i ){
        for(int j = -1;j <= 1;++j ){
            for(int k = -1;k <= 1;++k){
                ...
                //     std::cout << "Expanding Error" << std::endl;
                // }
...
double AstarPathFinder::getHeu(GridNodePtr node1, GridNodePtr node2)
{
    ...
    */
    ...
    // Manhattan
    // dist = std::abs(node1_coord(0) - node2_coord(0) ) + std::abs(node1_coord(1) - node2_coord(1) ) + std::abs(node1_coord(2) - node2_coord(2) );

    // Euclidean
    // dist = std::sqrt(std::pow((node1_coord(0) - node2_coord(0)), 2 ) + std::pow((node1_coord(1) - node2_coord(1)), 2 ) + std::pow((node1_coord(2) - node2_coord(2)), 2 ));
    ...
    // Diagonal distance
        ...
        // dist =std::sqrt(2.0)*diagonal + (dx + dy + dz- 2*diagonal);
        dist = dx + dy + dz + (std::sqrt(3.0) -3) * diagonal; 
