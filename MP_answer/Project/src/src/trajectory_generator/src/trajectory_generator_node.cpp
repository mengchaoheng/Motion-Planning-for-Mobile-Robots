...
double magic_fabian_constant=2.0;

// ros related
ros::Subscriber _map_sub, _pts_sub, _odom_sub;
ros::Publisher _traj_vis_pub, _traj_pub, _path_vis_pub, _vis_pub;
...
void execCallback(const ros::TimerEvent &e) {
  ...
  case REPLAN_TRAJ: {
    ...
    // cout << "start_vel"<<start_vel << endl;
    // start_pt = odom_pt;
    // start_vel = odom_vel;
    // cout << "odom_pt"<<odom_pt << endl;
...
bool trajGeneration() {
  ...
  //   cout << "grid_path"<<grid_path[k] << endl;
  // }

  vector<Vector3d>  grid_path_temp = _astar_path_finder->pathSimplify(grid_path, _path_resolution); //grid_path; //

  // for (int k = 0; k < int(grid_path_temp.size()); k++) {
  //   cout << "grid_path_temp.size()"<<grid_path_temp[k] << endl;
  // }
  // add mid points



  MatrixXd path(int(grid_path_temp.size()), 3);
  for (int k = 0; k < int(grid_path_temp.size()); k++) {
    path.row(k) = grid_path_temp[k];
    // cout << "k"<<k << endl;
    cout << "path.row(k)"<<grid_path_temp[k] << endl;
  }
...
Eigen::MatrixXd traj_Mav_Optimization(Eigen::MatrixXd Path)
{
	//=======================================================================
  int path_l = Path.rows();
  mav_trajectory_generation::Vertex::Vector vertices;
  const int dimension = 3;
  const int derivative_to_optimize = mav_trajectory_generation::derivative_order::SNAP;
  mav_trajectory_generation::Vertex start(dimension),middle(dimension),end(dimension);
  start.makeStartOrEnd(Path.row(0),derivative_to_optimize);
  // if(start_vel.norm()<1e-5)
  // {
  //   Eigen:: VectorXd V_line = Path.row(1) - Path.row(0);
  //   V_line = V_line/V_line.norm();
  //   start.addConstraint(mav_trajectory_generation::derivative_order::VELOCITY,V_line);
  // }
  // else
  // {
  ...
  for (int k = 1; k < path_l - 1; k++) {
    middle.addConstraint(mav_trajectory_generation::derivative_order::POSITION,Path.row(k));
    vertices.push_back(middle);
    // std::cout << "Path.row(k)"<<Path.row(k) << std::endl;
  }

  end.makeStartOrEnd(Path.row(path_l-1),derivative_to_optimize);
  vertices.push_back(end);
  // std::cout << "Path.row(path_l-1)"<<Path.row(path_l-1) << std::endl;

  //compute the segment times
  std::vector<double> segment_times;
  
  segment_times = estimateSegmentTimesNfabian(vertices , _Vel , _Acc , magic_fabian_constant);
  // std::cout <<"segment_times.size()"<< segment_times.size() << std::endl;
  // std::cout <<"_polyTime.size()"<< _polyTime.size() << std::endl;
  _polyTime.resize(segment_times.size());
  for (int i = 0; i < segment_times.size(); i ++)
  {
    _polyTime(i) = segment_times[i];
    // std::cout <<"segment_times"<< segment_times[i] << std::endl;
  ...
  opt.addMaximumMagnitudeConstraint(mav_trajectory_generation::derivative_order::VELOCITY, _Vel);
  opt.addMaximumMagnitudeConstraint(mav_trajectory_generation::derivative_order::ACCELERATION, _Acc);
  ...
  // mav_trajectory_generation::drawMavTrajectory(trajectory, distance, frame_id, &_markers);
  
// Class: rviz/MarkerArray
//           Enabled: true
//           Marker Topic: /trajectory_generator_node/trajectory_traject
//           Name: MarkerArray
//           Namespaces:
//             acceleration: true
//             path: true
...
//===========================_dev_order = 5 =========================
  ...
  int Di_seg = trajectory.D();
  int N_seg = trajectory.N();
  MatrixXd PolyCoeff_all(num_seg, Di_seg * N_seg);
  for(int i = 0; i < num_seg; i++){
    mav_trajectory_generation::Polynomial::Vector polynomials = segments[i].getPolynomialsRef();
    for(int j = 0; j < Di_seg; j++){
      Eigen::VectorXd Coeff = polynomials[j].getCoefficients();
      for(int k=0;k<N_seg;k++){
        PolyCoeff_all(i,k+N_seg*j)=Coeff(k);
      
      }
    }
  }
  // cout << "num_seg :" << endl << num_seg << endl;
  // cout << "N_seg :" << endl << N_seg << endl;
  // cout << "PolyCoeff_all :" << endl << PolyCoeff_all << endl;
  
  return PolyCoeff_all;
}
...
void trajOptimization(Eigen::MatrixXd path) {
  ...
  //============================= _dev_order = 5 ==========================
  _polyCoeff = traj_Mav_Optimization(path);

  // cout << "_polyCoeff :" << endl << _polyCoeff << endl;
  ...
  while (unsafe_segment != -1) {
     ...
     *
     * **/    
    // cout << "unsafe_segment :" << endl << unsafe_segment << endl;
  
    cout << "===================trajOptimization===================== " << endl;
    std::cout << "path"<<path << std::endl;
    std::cout << "path.rows()"<<path.rows() << std::endl;
    int num_seg = path.rows();
    
    repath.resize(num_seg+1,3);
    // repath.row(unsafe_segment+1) = (path.row(unsafe_segment) + path.row(unsafe_segment+1))/2;



    for(int i = 0; i < repath.rows(); i++){
      if(i<=unsafe_segment)
      {
        repath.row(i) = path.row(i);
      }else if(i>=unsafe_segment+2){
        repath.row(i) = path.row(i-1);
      }else{
        std::cout << "path.row(unsafe_segment)"<<path.row(unsafe_segment) << std::endl;
        std::cout << "path.row(unsafe_segment+1)"<<path.row(unsafe_segment+1) << std::endl;
        repath.row(i) = (path.row(unsafe_segment) + path.row(unsafe_segment+1))/2;
        std::cout << "repath.row(i)"<<repath.row(i) << std::endl;
      }
    }
    std::cout << "repath"<<repath << std::endl;
    std::cout << "repath.rows()"<<repath.rows() << std::endl;
    path.resize(num_seg+1,3);
    path = repath;

    _polyCoeff.resize(num_seg+1,3);
    _polyTime.resize(num_seg+1);
...
VectorXd timeAllocation(MatrixXd Path) {
  ...
  for(int k = 0;k < Path.rows()-1;k++){
      ...
      // }
      time(k) = s / _Vel * 2 *
               (1.0 + magic_fabian_constant * _Vel / _Acc *
                          exp(-s / _Vel * 2));
  }
  return time;
}
...
int main(int argc, char **argv) {
  ...
  nh.param("replanning/thresh_replan", replan_thresh, 2.0);
  nh.param("replanning/thresh_no_replan", no_replan_thresh, 2.0);
  ...
  // _vis_pub = nh.advertise<visualization_msgs::MarkerArray>( "trajectory_traject" , 10);
